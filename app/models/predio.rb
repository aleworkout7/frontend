	...
	def self.search(search)
		...
		if search.present?
			result = result	.joins("left join housing_sets hs on hs.id = predios.housing_set_id")
							...
							.where("lower(predios.name) like :search or lower(predios.address) like :search or lower(details) like :search or lower(hs.name) like :search or lower(d.name) like :search or lower(c.name) like :search", { search: "%#{search.downcase}%" })
							.distinct
		end
